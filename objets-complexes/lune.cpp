...
Lune::Lune(std::mt19937 &rg)
    : Astre{{util::alea(0, 1366, rg), util::alea(50, 250, rg)}},
      ...
      m_croissant{{m_coords.getX() + ((m_lune.getRayon()) / 2.5), m_coords.getY()}, m_lune.getRayon() / 1.3,
          {30, 144, 255}} {}//NUIT//{0, 0, 128}}//    25, 25, 112
...
///Apres avoir dessiner la Lune dans le svg.file on rajoute des effets de gradation
    for (int i = 1; i < 230; ++i)
        degrade.push_back(new Cercle{{m_coords}, m_lune.getRayon() - (i / 2),
            {240 - i, 248 - i, 255 - i}});//{255, 115+(i/2), 0}});
...
void Lune::VarColFonc(Heure &heure) {
    ...
    if (heure.getHeure() >= 7 && heure.getHeure() < 12)
        m_croissant.setCouleur({254 - 224 * (heure.getHeure() - 7) / (5), 155 - 11 * (heure.getHeure() - 7) / (5),
                                   70 + 185 * (heure.getHeure() - 7) / (5)});

    ///De 12h a 17h
    if (heure.getHeure() >= 12 && heure.getHeure() < 17)
        m_croissant.setCouleur({30 + 10 * (heure.getHeure() - 12) / (5), 144 + 46 * (heure.getHeure() - 12) / (5),
                                   255 - 0 * (heure.getHeure() - 12) / (5)});

    ///De 17h a 20h
    if (heure.getHeure() >= 17 && heure.getHeure() < 20)
        m_croissant.setCouleur({40 + 214 * (heure.getHeure() - 17) / (3), 190 - 35 * (heure.getHeure() - 17) / (3),
                                   255 - 185 * (heure.getHeure() - 17) / (3)});

    ///De 20h a 23h     00h etant le milieu de la nuit
    if (heure.getHeure() >= 20 && heure.getHeure() <= 23)
        m_croissant.setCouleur({254 - 202 * (heure.getHeure() - 20) / 4, 155 - 93 * (heure.getHeure() - 20) / 4,
                                   70 + 163 * (heure.getHeure() - 20) / 4});
    ///De 00h a 7h
    if (heure.getHeure() >= 0 && heure.getHeure() < 7)
        m_croissant.setCouleur({52 + 202 * (heure.getHeure() - 0) / 5, 62 + 93 * (heure.getHeure() - 0) / 5,
